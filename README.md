...
#### MySQL
	...
	블로그, 게시판 등 읽기 작업이 쓰기 작업보다 훨씬 많고 빠른 응답 속도가 중요한 경우 선택한다.
	문법이 간결하고 쉬우며, 메모리 사용량이 현저히 낮아서 부담없이 사용 가능하다.

	만약 복잡한 쿼리 및 분석 쿼리 비율이 높을 경우 PostgreSQL을 선택한다.

	DBMS 채택률
	2019년 MySQL 60% : PostgreSQL 30%
	2024년 MySQL 52% : PostgreSQL 45%
...
#### RDBMS(관계형 데이터베이스 시스템)
	...
	1    한동석   23   hds1234      20251211001    2    20251211    54
	2    이대흥   21   ldh6666      20251212001    2    20251212    2
	3    안채현   15   ach7777      20251212002    4    20251212    3
	4    정보성   55   jbs9999      20251215001    1    20251215    67
...
#### 3차 정규화
	PK가 아닌 컬럼이 다른 컬럼을 결정하는 경우(이행 종속 제거)

	회원번호   이름    시      구      동       우편번호
	1        한동석   서울시   강남구   역삼동    12345
	2        홍길동   서울시   관악구   봉천동    45678
	3        이순신   남양주   화도읍   구암리    78787
...
#### 이상현상의 종류
	...
	3. 삭제 이상
		...
		"이순신"이 담당한 프로젝트에 문제가 생겨 드랍된다면, "이순신" 행을 모두 삭제하게 된다.
...
#### DML(Data Manipulation Language) : 데이터 조작어
	...
	2. insert : 추가
		1) 컬럼을 골라서 적을 수 있다. 이 때 전달하지 않은 값은 default 제약조건이 발동된다.
			insert into [테이블명] ([컬럼명1], [컬럼명2], ...)
			values([값1], [값2], ...)

		2) 모든 값을 전부 작성해야되며, 컬럼명은 직접 작성하지 않는다.
			insert into [테이블명]
			values([값1], [값2], ...)
...
#### Join
	...
	가상의 테이블로 만들고 결과를 보여주는 문법.
	정규화를 통해 조회 테이블이 쪼개져 있으면 한번에 모든 정보를 가져올 수 없기 때문에
	이를 Join으로 해결할 수 있다.
	...
	1. 내부조인 (inner join)
		...
		- 등가 조인
			on 절의 조건식에 등호가 있는 join

		- 비등가 조인
			on 절의 조건식에 등호가 없는 join

	2. 외부조인 (outer join)
		조건이 일치하지 않아도 원하는 테이블을 모두 조회
...
#### TCL(Transaction Control Language) : 트랜잭션 제어어
	...
	이 때, INSERT는 잘 되었어도 UPDATE에서 문제가 발생하면,
...
#### View 문법
	...
	- 새롭게 생성 혹은 기존 View 수정
		create or replace [뷰 이름] as (select 쿼리문);
